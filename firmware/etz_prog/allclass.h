...
public:
  ...
  Motor(const char* mName, int dir, int step, int dir2, int step2, Point start, OLEDMenu* display, bool forward = 0, bool forward2 = 1) {
    name = mName;
    DIRPIN = dir;
    STEPPIN = step;
    DIRPIN2 = dir2;
...
public:
  Paint(int servoPin, int dir, int step, Point startPos, int needPlus, OLEDMenu* display) {
    ...
    start = startPos;
    need = needPlus;
    motors = Motor("motors", 41, 39, 69, 67, startPos, display);
  }
  void initServo();
  void initYStepper();
  void initDrive();
